...
export const Testimonials = () => {
  return (
    <section id="testimonials" className="relative py-24 lg:py-32 bg-elevated overflow-hidden">

      {/* Large decorative quote marks */}
      <div className="absolute left-4 top-12 font-display text-[18rem] leading-none text-watermark pointer-events-none select-none">
        "
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {testimonials.map((t, i) => (
            <Reveal key={t.name} delay={i * 80}>
              <div className="group h-full flex flex-col p-7 rounded-xl border border-theme bg-card hover:bg-card-hover hover:border-[#ce0000]/20 hover:shadow-xl hover:shadow-[#ce0000]/5 transition-all duration-300">
                ...
                {/* Quote */}
                <p className="font-sans text-secondary text-sm leading-relaxed flex-1 mb-6 italic">
                  "{t.text}"
                </p>

                {/* Author */}
                <div className="flex items-center gap-3 pt-5 border-t border-theme">
                  <div className="w-9 h-9 rounded-lg bg-zinc-800 border border-theme flex items-center justify-center font-condensed font-bold text-white text-xs shrink-0">
                    {t.initials}
                  </div>
                  <div>
                    <p className="font-condensed font-bold text-primary text-sm uppercase tracking-wide">
                      {t.name}
                    </p>
